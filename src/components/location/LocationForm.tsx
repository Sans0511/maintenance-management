...
type Props = {
  inputData: LocationAttributes
  isOpen: boolean
  onClose: () => void
  onSubmit: () => void
}

export default function LocationForm({ inputData, isOpen, onClose, onSubmit }: Props) {
  const [formData, setFormData] = useState<LocationAttributes>(inputData)
      ...
      <div className="no-scrollbar relative w-full max-w-[600px] overflow-y-auto rounded-3xl bg-white p-4 lg:p-11 dark:bg-gray-900">
        <div className="mb-6 flex items-center justify-between">
          <h1 className="text-xl font-semibold text-gray-800 dark:text-white/90">
            {inputData?.id ? 'Update Location' : 'Create Location'}
          </h1>
        </div>
