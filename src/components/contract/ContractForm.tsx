...
type Props = {
  inputData: ContractAttributes
  isOpen: boolean
  onClose: () => void
  onSubmit: () => void
}

export default function ContractForm({ inputData, isOpen, onClose, onSubmit }: Props) {
  const [formData, setFormData] = useState<ContractAttributes>(inputData)
      ...
      <div className="no-scrollbar relative w-full max-w-[600px] overflow-y-auto rounded-3xl bg-white p-4 lg:p-11 dark:bg-gray-900">
        <div className="mb-6 flex items-center justify-between">
          <h1 className="text-xl font-semibold text-gray-800 dark:text-white/90">
            {inputData?.id ? 'Update Contract' : 'Create Contract'}
          </h1>
        </div>
        ...
        <form onSubmit={handleOnSubmit}>
          <div className="space-y-5">
            <div>
              <Label>
                Contract Name<span className="text-error-500">*</span>
              </Label>
              <Input
                type="text"
                name="contractName"
