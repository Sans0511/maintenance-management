...
type Props = {
  inputData: DesignationAttributes
  isOpen: boolean
  onClose: () => void
  onSubmit: () => void
}

export default function DesignationForm({ inputData, isOpen, onClose, onSubmit }: Props) {
  const [formData, setFormData] = useState<DesignationAttributes>(inputData)
      ...
      <div className="no-scrollbar relative w-full max-w-[600px] overflow-y-auto rounded-3xl bg-white p-4 lg:p-11 dark:bg-gray-900">
        <div className="mb-6 flex items-center justify-between">
          <h1 className="text-xl font-semibold text-gray-800 dark:text-white/90">
            {inputData?.id ? 'Update Designation' : 'Create Designation'}
          </h1>
        </div>
        ...
        <form onSubmit={handleOnSubmit}>
          <div className="space-y-5">
            <div>
              <Label>
                Designation Name<span className="text-error-500">*</span>
              </Label>
              <Input
                type="text"
