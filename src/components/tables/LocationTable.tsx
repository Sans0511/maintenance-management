...
import { LocationAttributes } from '@/lib/types'
import { useModal } from '@/hooks/useModal'
import { Pencil } from 'lucide-react'
import LocationForm from '@/components/location/LocationForm'

const initialFormData: LocationAttributes = {
  locationName: '',
  status: 'ACTIVE',
}
...
export default function LocationTable() {
  ...
  const [data, setData] = useState<LocationAttributes[]>([])
  const [loading, setLoading] = useState(true)
  const [totalCount, setTotalCount] = useState(0)
  const { isOpen, openModal, closeModal } = useModal()
  const [formData, setFormData] = useState<LocationAttributes>(initialFormData)
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        ...
        const res = await axios.get(`/api/locations?skip=${skip}&limit=${itemsPerPage}`)
  ...
  const handleUpdate = (item: LocationAttributes) => {
    setFormData(item)
    openModal()
  }
          ...
          {loading ? (
            ...
            <>
              <div className="flex items-center justify-between px-5 py-4">
                <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-300">Locations</h2>
                <button
              ...
              <Table>
                <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                  <TableRow>
                    ...
                    <TableCell isHeader className="table-heading">Location Name</TableCell>
                ...
                <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                  {data.map((item, index) => (
                    <TableRow key={item.id ?? index}>
                      ...
                      <TableCell className="table-data">{item.locationName}</TableCell>
