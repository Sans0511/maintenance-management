...
const initialFormData: DepartmentAttributes = {
  departmentName: '',
  status: 'ACTIVE',
}
...
export default function DepartmentTable() {
  ...
  const [data, setData] = useState<DepartmentAttributes[]>([])
  const [loading, setLoading] = useState(true)
  const [totalCount, setTotalCount] = useState(0)
  const { isOpen, openModal, closeModal } = useModal()
  const [formData, setFormData] = useState<DepartmentAttributes>(initialFormData)
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      try {
        ...
        const res = await axios.get(`/api/departments?skip=${skip}&limit=${itemsPerPage}`)
  ...
  const handleUpdate = (item: DepartmentAttributes) => {
    setFormData(item)
    openModal()
  }
          ...
          {loading ? (
            ...
            <>
              <div className="flex items-center justify-between px-5 py-4">
                <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-300">Departments</h2>
                <button
              ...
              <Table>
                <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                  <TableRow>
                    ...
                    <TableCell isHeader className="table-heading">Department Name</TableCell>
