...
import { DesignationAttributes } from '@/lib/types'
import { useModal } from '@/hooks/useModal'
import { Pencil } from 'lucide-react'
import DesignationForm from '@/components/designation/DesignationForm'

const initialFormData: DesignationAttributes = {
  designationName: '',
  status: 'ACTIVE',
}
...
export default function DesignationTable() {
  ...
  const [data, setData] = useState<DesignationAttributes[]>([])
  const [loading, setLoading] = useState(true)
  const [totalCount, setTotalCount] = useState(0)
  const { isOpen, openModal, closeModal } = useModal()
  const [formData, setFormData] = useState<DesignationAttributes>(initialFormData)
  ...
  useEffect(() => {
    const fetchDesignations = async () => {
      try {
        ...
        const res = await axios.get(`/api/designations?skip=${skip}&limit=${itemsPerPage}`)
  ...
  const handleUpdate = (item: DesignationAttributes) => {
    setFormData(item)
    openModal()
  }
          ...
          {loading ? (
            ...
            <>
              <div className="flex items-center justify-between px-5 py-4">
                <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-300">Designations</h2>
                <button
              ...
              <Table>
                <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                  <TableRow>
                    ...
                    <TableCell isHeader className="table-heading">Designation Name</TableCell>
